...
from core.orpheus_exceptions import BadStateError, NotImplementedError, BadParametersError
from core.relation import RelationManager
from core.db import DatabaseManager
import core.orpheus_const as const

from vgraph import VersionGraph
	...
	def parse(self, cmd_string, explain_btn):
		...
		try:
			if action == "init":
				...
				parent_name, curt_vid, parent_list = executor.exec_commit(message, table_name, file_name, delimiters, header)
				...
				attr_names, transactions = executor.exec_run(sql)
				table_list = []
				table_list.append((attr_names, transactions))
				return table_list
			elif action == "explain":
				sql = self.__parse_run(cmd)
				return executor.exec_explain(sql)
			elif action == "drop":
				...
				return executor.exec_show(dataset)
 			elif action == "restore":
 				executor.exec_restore()
			else:
