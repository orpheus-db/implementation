...
class VersionManager(object):
	...
	def init_version_graph_dataset(self, dataset, list_of_rid, user):
		...
		version_table = const.getPublicSchema() + const.getVersiontableName(dataset)
		init_version_sql = "INSERT INTO %s VALUES (1, '%s', %s,'{0}', '{-1}', '{}', '%s', '%s', 'init commit');" \
							% (version_table, user, str(len(list_of_rid)), str(datetime.datetime.now()), str(datetime.datetime.now()))
		self.conn.cursor.execute(init_version_sql)
		self.conn.connect.commit()

	def init_index_table_dataset(self, dataset, list_of_rid):
		print "Initializing index table"
		self.conn.refresh_cursor()
		index_table = const.getPublicSchema() + const.getIndextableName(dataset)
		# insert into indexTbl values ('{1,3,5}', '{1}')
		init_indextbl_sql = "INSERT INTO %s VALUES \
							 (1, '{%s}');" % (index_table, str(",".join(map(str, list_of_rid))))
		self.conn.cursor.execute(init_indextbl_sql)
		self.conn.connect.commit()

	def update_version_graph(self, version_graph_name, user, num_of_records, commonRCnt_list, parent_list, table_create_time, msg):
		# print "update_version_graph"
		# create new version
		parent_list_string = '\'{' + ', '.join(parent_list) + '}\'' 
		commonRCnt_string = '\'{' + ', '.join(map(str, commonRCnt_list)) + '}\''
		...
		curt_vid = max_vid + 1
		values = "(%s, '%s', %s, %s, %s, %s, %s, %s, %s)" % (curt_vid, user, num_of_records, commonRCnt_string, parent_list_string, "'{}'", "'%s'" % table_create_time, "'%s'" % commit_time, "'%s'" % msg)
