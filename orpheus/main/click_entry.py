...
def init(ctx, input, dataset, table, schema):
	...
	try:
		...
		pid = 1
		# create new dataset
		conn.create_dataset(input, dataset, schema_tuple, pid, attributes=attribute_name)
		datatable_name = const.getPublicSchema() + const.getDatatableName(dataset, pid)
		# get all rids in list
		lis_rid = rel.select_all_rid(datatable_name)
		...
		partitioner = Partitioner(conn, dataset)
		partitioner.partition_init()

		# set the partition delta of the current dataset as 1
		# TODO: But more complicated than needed, refactor the meta class
		metadata = MetadataManager(ctx.obj)
		meta_obj = metadata.load_meta()
		metadata.update_delta(dataset, 1.0, meta_obj)
...
def checkout(ctx, dataset, vlist, to_table, to_file, delimiters, header, ignore):
	...
	try:
		metadata = MetadataManager(ctx.obj)
		meta_obj = metadata.load_meta()
		indextable = const.getIndextableName(dataset)
		relation.checkout(vlist, dataset, indextable, to_table=to_table, to_file=abs_path, delimiters=delimiters, header=header, ignore=ignore)
...
def commit(ctx, msg, table_name, file_name, delimiters, header):
	...
	parent_name = parent_vid_list[0] # dataset name
	parent_list = parent_vid_list[1] # a list of parent vids

	plist = Partitioner.plist_lookup(parent_list, conn, parent_name)
	vplist = zip(parent_list, plist) # (vid, pid)
	sample_datatable = const.getDatatableName(parent_name, vplist[0][1])

	indextable_name = const.getIndextableName(parent_name)
	graph_name = const.getVersiontableName(parent_name)

	try:
		# convert file into tmp_table first, then set the table_name to tmp_table
		if file_name:
			# need to know the schema for this file
			_attributes, _attributes_type = relation.get_datatable_attribute(sample_datatable)

			relation.create_relation_force('tmp_table', sample_datatable, sample_table_attributes=_attributes) # create a tmp table
	...
	if table_name:
		try:
			...
			vrlist = dict() # vid -> list of common record
			for vp in vplist:
				vid, pid = vp[0], vp[1]
				datatable_name = const.getDatatableName(parent_name, pid)
				view_name = "%s_view_v%s" % (parent_name, vid)
				relation.create_parent_view(datatable_name, indextable_name, vid, view_name)
				existing_rids = [t[0] for t in relation.select_intersection_table(table_name, view_name, commit_attributes)]
				vrlist[vid] = existing_rids
				relation.drop_view(view_name)

			# Update the version table
			commonRCnt_list = [len(rlist) for rlist in vrlist.values()]
			rCnt = relation.get_num_tuples(table_name)
			table_create_time = metadata.load_table_create_time(table_name) if table_name != 'tmp_table' else None
			curt_vid = version.update_version_graph(graph_name, ctx.obj['user'], rCnt, commonRCnt_list, parent_list, table_create_time, msg)

			# Assign to the right partition. May trigger the migration engine
			storage_threshold, tolerance = ctx.obj['storage_threshold'], ctx.obj['tolerance']
			p = Partitioner(conn, parent_name, storage_threshold, tolerance)
			cur_delta = metadata.load_cur_delta(parent_name)
			new_pid, parent_pid, parent_vid, new_delta = p.partition_assign(curt_vid, cur_delta)

			if new_pid != parent_pid:
				datatable_name = const.getDatatableName(parent_name, new_pid)
				sql = "select %s from %s" % (','.join(_attributes), table_name)
			else:
				parent_datatablename = const.getDatatableName(parent_name, parent_pid)
				view_name = "%s_view_v%s" % (parent_name, parent_vid)
				relation.create_parent_view(parent_datatablename, indextable_name, parent_vid, view_name)

				# find the new records #TODO
			...
			current_version_rid = new_rids
			if new_pid == parent_pid:
				common_rlist = vrlist[parent_vid]
				current_version_rid += common_rlist

			# print "Found %s new records" % len(new_rids)
			# print "Found %s existing records" % len(common_rlist)

			# update index table
			version.update_index_table(indextable_name, curt_vid, current_version_rid)

			# update partition delta
			# TODO: refactor metadata class
			if (cur_delta != new_delta):
				meta_obj = metadata.load_meta()
				metadata.update(parent_name, new_delta, meta_obj)
				metadata.commit_meta(meta_obj)
